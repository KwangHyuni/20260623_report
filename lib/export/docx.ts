...
export async function generateDocxBuffer(
  ...
  const children: (Paragraph | Table)[] = [
    ...
    bullet(`총 매출: ${formatKRW(metrics.kpi.totalRevenue)}`),
    bullet(`주문 건수: ${formatNumber(metrics.kpi.orderCount)}건`),
    bullet(`평균 주문금액: ${formatKRW(metrics.kpi.avgOrderAmount)}`),
    bullet(`활성 고객 수: ${formatNumber(metrics.kpi.activeCustomerCount)}명`),
